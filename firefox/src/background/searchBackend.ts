...
export async function updateIndex() {
  ...
  const appSetting = (await browser.storage.local.get('arcacon_setting')).arcacon_setting ?? [];
  const packageList = (await browser.storage.local.get('arcacon_package')).arcacon_package ?? [];
